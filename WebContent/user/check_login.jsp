<%@page import="kr.co.koo.taeyeon.member.model.MemberVO"%>
...
<%
	...
	if(rn == MemberDAO.LOGIN_SUCCESS){ // id, pw가 일치한다면
		...
		session.setAttribute("user_id", id); // user_id라는 이름으로 세션 생성 (회원임을 증명하기 위함)
		session.setAttribute("user_name", user.getUserName()); // user_name라는 이름으로 세션 생성
		response.sendRedirect("/WebProject-1"); // 홈으로 이동시키기.
%>		
